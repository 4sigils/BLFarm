--[[
    ...
    USAGE:
      local Win = GalaxLib:CreateWindow({ Title="Hub", Size=Vector2.new(580,480), MenuKey=0x70 })
...
local function tW(s,sz)     return #(s or "")*(sz or 13)*0.6 end

local _UIS = game:GetService("UserInputService")
local function mpos()
    local ok,v = pcall(function() return _UIS:GetMouseLocation() end)
...
pcall(function()
    _UIS.InputChanged:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseWheel then
            _scrollDelta=_scrollDelta+(i.Position.Z>0 and -1 or 1)
        end
    end)
end)
...
function Input:update()
    ...
    self.click=m1 and not(self._prev.m1 or false); self.held=m1; self._prev.m1=m1
    self.scroll=_scrollDelta; _scrollDelta=0
end
function Input:keyClick(kc)
    local c=iskeypressed and iskeypressed(kc) or false
    local p=self._prev[kc] or false; self._prev[kc]=c; return c and not p
end
...
local function renderNotifs()
    ...
    for i,n in ipairs(_notifs) do
        local nx,ny=1920-NW-10, 1080-i*(NH+NG)-10
        local a=n.alpha
        ...
        local nid="__n"..n.id
        poolAdd(_notifPool,nid.."bg", "Square",{Position=Vector2.new(nx,ny),  Size=Vector2.new(NW,NH),  Filled=true, Color=bg,Visible=true,ZIndex=200})
        poolAdd(_notifPool,nid.."bd", "Square",{Position=Vector2.new(nx,ny),  Size=Vector2.new(NW,NH),  Filled=false,Color=ac,Thickness=1, Visible=true,ZIndex=201})
        poolAdd(_notifPool,nid.."bar","Square",{Position=Vector2.new(nx,ny),  Size=Vector2.new(3,NH),   Filled=true, Color=ac,Visible=true,ZIndex=202})
        ...
        poolAdd(_notifPool,nid.."prg","Square",{Position=Vector2.new(nx,ny+NH-3),Size=Vector2.new(math.max(0,NW*pct),3),Filled=true,Color=ac,Visible=true,ZIndex=202})
    end
    poolFlush(_notifPool)
end

-- ── Section builder (shared between tabs and settings) ───────────────────────
local function buildSection(sname, parentList)
    ...
    function SEC:AddMultiDropdown(label,options,default,opts2,cb)
        ...
        return{Get=function()return lst()end,
               Set=function(_,tbl)it.selected={};if tbl then for _,v in ipairs(tbl) do it.selected[v]=true end end;it.cb(lst())end}
    end
    function SEC:AddColorPicker(label,default,cb)
        local h,s,v=rgbToHsv(default or Color3.new(1,0,0))
        local it={type="colorpicker",label=label,value=default or Color3.new(1,0,0),
            h=h,s=s,v=v,cb=cb or function()end,dragSV=false,dragH=false,_open=false}
        reg(it); it.cb(it.value)
        return{Get=function()return it.value end,
    ...
    function SEC:AddSeparator() reg({type="separator"}); return{} end

    table.insert(parentList,SEC); return SEC
end
...
function GalaxLib:CreateWindow(opts)
    opts=opts or {}
    local WIN={
        Title   = opts.Title   or "GalaxLib",
        MenuKey = opts.MenuKey or 0x70,
        Size    = Vector2.new(math.max(580, opts.Size and opts.Size.X or 580),
                              math.max(480, opts.Size and opts.Size.Y or 480)),
    ...
    function WIN:AddTab(name)
        ...
        function TAB:AddSection(sname) return buildSection(sname,self._sections) end
    ...
    function WIN:_buildSettings()
        ...
        function ST:AddSection(sname) return buildSection(sname,self._sections) end
        ...
        table.insert(STH._widgets,{type="dropdown",label="Theme",
    ...
    -- ── Dropdown list ─────────────────────────────────────────────────────────
    function WIN:_ddList(pool,wid,it,ddPos,ddSz,iW,F,multi)
        local optH=20; local SBW=5; local sbH=26
        ...
        local total=#filt; local visN=math.min(it.maxVisible,total)
        it.scroll=clamp(it.scroll,0,math.max(0,total-visN))
        local hasSB=(total>it.maxVisible)
        local optW=hasSB and (iW-SBW-4) or iW
        local lPos=ddPos+Vector2.new(0,ddSz.Y+2); local lH=visN*optH+sbH+4

        poolAdd(pool,wid.."dll","Square",{Position=lPos,Size=Vector2.new(iW,lH),Filled=true, Color=T.Surface0,Visible=true,ZIndex=20})
        poolAdd(pool,wid.."dlb","Square",{Position=lPos,Size=Vector2.new(iW,lH),Filled=false,Color=T.Accent, Thickness=1,Visible=true,ZIndex=21})

        -- Search box
        local sbPos=lPos+Vector2.new(4,4); local sbSz=Vector2.new(iW-8,sbH-8)
        if Input.click then
            if over(sbPos,sbSz) then it._sfocus=true
            elseif it._sfocus   then it._sfocus=false end
        end
        poolAdd(pool,wid.."dsb", "Square",{Position=sbPos,Size=sbSz,Filled=true, Color=T.Surface1,Visible=true,ZIndex=22})
        poolAdd(pool,wid.."dsbb","Square",{Position=sbPos,Size=sbSz,Filled=false,Color=it._sfocus and T.Accent or T.Border1,Thickness=1,Visible=true,ZIndex=23})
        local cur=(it._sfocus and floor(tick()*2)%2==0) and "|" or ""
        local sdsp=it._search~="" and (it._search..cur) or (it._sfocus and cur or "search...")
        poolAdd(pool,wid.."dsbt","Text",{Position=sbPos+Vector2.new(5,2),Text=sdsp,Size=12,Font=F,
                ...
                if Input:keyClick(kc) then
                    ...
                    elseif kc>=0x30 and kc<=0x5A then
                        local ch=KeyNames[kc]; if ch and #ch==1 then
                            ...
                            it._search=it._search..(sh and (ShiftMap[ch:lower()] or ch:upper()) or ch:lower())
                        end
        ...
        if hasSB then
            ...
            local barH=math.max(14,(lH-sbH)*(it.maxVisible/total))
            local trackH=(lH-sbH)-barH
            local barY=lPos.Y+sbH+trackH*(it.scroll/maxSc)
            local barX=lPos.X+iW-SBW-2
            local bPos=Vector2.new(barX,barY); local bSz=Vector2.new(SBW,barH)
            local bHov=over(bPos,bSz)
            poolAdd(pool,wid.."sbtrk","Square",{Position=Vector2.new(barX,lPos.Y+sbH),Size=Vector2.new(SBW,lH-sbH),Filled=true,Color=T.Surface1,Visible=true,ZIndex=21})
            poolAdd(pool,wid.."sbbar","Square",{Position=bPos,Size=bSz,Filled=true,Color=(bHov or it._sbDrag) and T.Text or T.Accent,Visible=true,ZIndex=22})
            ...
            if it._sbDrag and trackH>0 then
                ...
                it.scroll=clamp((it._sbDragSc or 0)+floor(dy*(maxSc/trackH)+0.5),0,maxSc)
            end
        end
        if over(lPos,Vector2.new(iW,lH)) and Input.scroll~=0 then
            it.scroll=clamp(it.scroll+Input.scroll,0,math.max(0,total-it.maxVisible))
        end

        -- Options
        for vi=1,visN do
            local opt=filt[vi+it.scroll]; if not opt then break end
            local opPos=lPos+Vector2.new(0,(vi-1)*optH+sbH+2)
            local opSz=Vector2.new(optW,optH); local hov=over(opPos,opSz)
            if multi then
                local sel=it.selected[opt]==true
                if hov then poolAdd(pool,wid.."ohi"..vi,"Square",{Position=opPos,Size=opSz,Filled=true,Color=T.Surface1,Visible=true,ZIndex=21})
                else poolHide(pool,wid.."ohi"..vi) end
                local cp=opPos+Vector2.new(6,5); local cs=Vector2.new(10,10)
                poolAdd(pool,wid.."mcb"..vi, "Square",{Position=cp,Size=cs,Filled=true, Color=sel and T.Accent or T.Surface0,Visible=true,ZIndex=22})
                poolAdd(pool,wid.."mcbb"..vi,"Square",{Position=cp,Size=cs,Filled=false,Color=T.Border1,Thickness=1,Visible=true,ZIndex=23})
                poolAdd(pool,wid.."mot"..vi, "Text",  {Position=opPos+Vector2.new(22,3),Text=opt,Size=13,Font=F,Color=sel and T.Accent or T.Text,Outline=false,Visible=true,ZIndex=22})
                if it._clicked and hov then
                ...
                if hov or sel then poolAdd(pool,wid.."ohi"..vi,"Square",{Position=opPos,Size=opSz,Filled=true,Color=sel and T.AccentDark or T.Surface1,Visible=true,ZIndex=21})
                else poolHide(pool,wid.."ohi"..vi) end
                poolAdd(pool,wid.."ot"..vi,"Text",{Position=opPos+Vector2.new(8,3),Text=opt,Size=13,Font=F,Color=sel and T.Accent or T.Text,Outline=false,Visible=true,ZIndex=22})
                if it._clicked and hov then it.value=opt;it.cb(opt);self._openDropId=nil;it._search="" end
            end
        end
        for vi=visN+1,it.maxVisible+4 do
            poolHide(pool,wid.."ohi"..vi); poolHide(pool,wid.."ot"..vi)
            poolHide(pool,wid.."mcb"..vi); poolHide(pool,wid.."mcbb"..vi); poolHide(pool,wid.."mot"..vi)
        end
        return lH+4
    end
    ...
    function WIN:_widget(it,pool,wid,wx,wy,iW,F)
        ...
        if it.type=="separator" then
            poolAdd(pool,wid.."s","Square",{Position=Vector2.new(wx,wy+5),Size=Vector2.new(iW,1),Filled=true,Color=T.Border0,Visible=true,ZIndex=6})
            return 12

        elseif it.type=="label" then
            poolAdd(pool,wid.."l","Text",{Position=Vector2.new(wx,wy+2),Text=it.label,Size=13,Font=F,Color=it.color or T.SubText,Outline=false,Visible=true,ZIndex=6})
            return 20

        elseif it.type=="toggle" then
            it._anim=lerp(it._anim,it.value and 1 or 0,0.22)
            local a=it._anim; local tw2,th2=28,14
            local trkP=Vector2.new(wx+iW-tw2,wy+1)
            poolAdd(pool,wid.."trk", "Square",{Position=trkP,Size=Vector2.new(tw2,th2),Filled=true, Color=lerpC(T.Surface1,T.Accent,a),Visible=true,ZIndex=6})
            poolAdd(pool,wid.."trkb","Square",{Position=trkP,Size=Vector2.new(tw2,th2),Filled=false,Color=lerpC(T.Border1,T.Accent,a),Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."knb", "Square",{Position=Vector2.new(trkP.X+2+a*(tw2-14),trkP.Y+2),Size=Vector2.new(10,10),Filled=true,Color=T.Text,Visible=true,ZIndex=8})
            poolAdd(pool,wid.."lbl", "Text",  {Position=Vector2.new(wx,wy+1),Text=it.label,Size=13,Font=F,Color=it.value and T.Text or T.SubText,Outline=false,Visible=true,ZIndex=6})
        ...
        elseif it.type=="button" then
            local bp=Vector2.new(wx,wy); local bs=Vector2.new(iW,24); local hov=over(bp,bs)
            local bgC,bdC
            if it.color then bgC=hov and lerpC(it.color,T.Text,0.2) or lerpC(it.color,T.Body,0.3); bdC=it.color
            else bgC=hov and T.AccentDark or T.Surface1; bdC=hov and T.Accent or T.Border0 end
            poolAdd(pool,wid.."btn", "Square",{Position=bp,Size=bs,Filled=true, Color=bgC,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."btnb","Square",{Position=bp,Size=bs,Filled=false,Color=bdC,Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."btnt","Text",  {Position=Vector2.new(wx+iW/2-tW(it.label,13)/2,wy+5),Text=it.label,Size=13,Font=F,Color=T.Text,Center=false,Outline=false,Visible=true,ZIndex=7})
            if Input.click and hov then it.cb() end
            return 28

        elseif it.type=="slider" then
            local vs=tostring(it.value)..it.suffix
            poolAdd(pool,wid.."lbl","Text",{Position=Vector2.new(wx,wy),Text=it.label,Size=13,Font=F,Color=T.Text,  Outline=false,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."val","Text",{Position=Vector2.new(wx+iW-tW(vs,12),wy),Text=vs,Size=12,Font=F,Color=T.Accent,Outline=false,Visible=true,ZIndex=6})
            local tP=Vector2.new(wx,wy+17); local tS=Vector2.new(iW,5)
            poolAdd(pool,wid.."trk","Square",{Position=tP,Size=tS,Filled=true,Color=T.Surface1,Visible=true,ZIndex=6})
            local pct=clamp((it.value-it.min)/(it.max-it.min),0,1)
            local fw=math.max(4,tS.X*pct)
            poolAdd(pool,wid.."fil","Square",{Position=tP,Size=Vector2.new(fw,tS.Y),Filled=true,Color=T.Accent,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."knb","Square",{Position=Vector2.new(tP.X+fw-4,tP.Y-3),Size=Vector2.new(8,11),Filled=true,Color=T.Text,Visible=true,ZIndex=8})
            ...
            if self._sliderDrag==it then
                if Input.held then
                    ...
                    it.value=clamp(floor(it.min+(it.max-it.min)*p+0.5),it.min,it.max);it.cb(it.value)
        ...
        elseif it.type=="dropdown" or it.type=="multidropdown" then
            ...
            local m1=Input.held; it._clicked=m1 and not it._wasM1; it._wasM1=m1
            ...
            local estLH=math.min(it.maxVisible,#it.options)*20+4+26

            poolAdd(pool,wid.."ddlbl","Text",   {Position=Vector2.new(wx,wy),Text=it.label,Size=12,Font=F,Color=T.SubText,Outline=false,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."ddbg", "Square", {Position=ddP,Size=ddS,Filled=true, Color=T.Surface1,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."ddb",  "Square", {Position=ddP,Size=ddS,Filled=false,Color=isOpen and T.Accent or T.Border0,Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."ddv",  "Text",   {Position=ddP+Vector2.new(6,4),Text=disp,Size=13,Font=F,Color=T.Text,Outline=false,Visible=true,ZIndex=7})
            local ax,ay=ddP.X+ddS.X-14,ddP.Y+11
            poolAdd(pool,wid.."ddarr","Triangle",{PointA=Vector2.new(ax,ay-4),PointB=Vector2.new(ax+7,ay-4),PointC=Vector2.new(ax+3.5,ay+3),Filled=true,Color=isOpen and T.Accent or T.SubText,Visible=true,ZIndex=7})

            local lPos=ddP+Vector2.new(0,ddS.Y+2)
            if it._clicked then
                ...
                elseif isOpen and not over(lPos,Vector2.new(iW,estLH)) then self._openDropId=nil end
            end
            local h=42
            if self._openDropId==it._sid then h=h+self:_ddList(pool,wid,it,ddP,ddS,iW,F,multi) end
            return h

        elseif it.type=="colorpicker" then
            local swP=Vector2.new(wx,wy)
            poolAdd(pool,wid.."sw",  "Square",{Position=swP,Size=Vector2.new(20,18),Filled=true, Color=it.value,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."swb", "Square",{Position=swP,Size=Vector2.new(20,18),Filled=false,Color=T.Border1,Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."lbl", "Text",  {Position=swP+Vector2.new(26,2),Text=it.label,Size=13,Font=F,Color=T.Text,Outline=false,Visible=true,ZIndex=6})
            local r,g,b=floor(it.value.R*255),floor(it.value.G*255),floor(it.value.B*255)
            local rgb=r..","..g..","..b
            poolAdd(pool,wid.."rgb","Text",{Position=Vector2.new(wx+iW-tW(rgb,11),wy+2),Text=rgb,Size=11,Font=F,Color=T.SubText,Outline=false,Visible=true,ZIndex=6})
            if Input.click and over(swP,Vector2.new(iW,18)) then it._open=not it._open end
            local h=24
            if it._open then
                local pW,pH=iW,90; local pPos=Vector2.new(wx,wy+24); local strips=16
                for si=1,strips do
                    local vv=1-(si-1)/(strips-1)
                    poolAdd(pool,wid.."sv"..si,"Square",{
                ...
                poolAdd(pool,wid.."palb","Square",{Position=pPos,Size=Vector2.new(pW,pH),Filled=false,Color=T.Border1,Thickness=1,Visible=true,ZIndex=10})
                poolAdd(pool,wid.."csh","Line",{From=Vector2.new(pPos.X,pPos.Y+(1-it.v)*pH),To=Vector2.new(pPos.X+pW,pPos.Y+(1-it.v)*pH),Thickness=1,Color=T.Text,Visible=true,ZIndex=9})
                poolAdd(pool,wid.."csv","Line",{From=Vector2.new(pPos.X+it.s*pW,pPos.Y),To=Vector2.new(pPos.X+it.s*pW,pPos.Y+pH),Thickness=1,Color=T.Text,Visible=true,ZIndex=9})
                local hH=12; local hPos=Vector2.new(wx,wy+24+pH+5); local hSegs=24
                for hi=1,hSegs do
                    poolAdd(pool,wid.."h"..hi,"Square",{
                ...
                poolAdd(pool,wid.."hcur","Square",{Position=Vector2.new(hPos.X+it.h*iW-2,hPos.Y-1),Size=Vector2.new(4,hH+2),Filled=false,Color=T.Text,Thickness=1,Visible=true,ZIndex=9})
                ...
                if it.dragSV then local m=mpos();it.s=clamp((m.X-pPos.X)/pW,0,1);it.v=1-clamp((m.Y-pPos.Y)/pH,0,1);it.value=hsvToRgb(it.h,it.s,it.v);it.cb(it.value) end
                if it.dragH  then it.h=clamp((mpos().X-hPos.X)/iW,0,1);it.value=hsvToRgb(it.h,it.s,it.v);it.cb(it.value) end
                h=h+pH+hH+12
            end
            return h
        ...
        elseif it.type=="keybind" then
            ...
            local kw=tW(ks,12)+10; local kp=Vector2.new(wx+iW-kw,wy); local ks2=Vector2.new(kw,18)
            poolAdd(pool,wid.."lbl", "Text",  {Position=Vector2.new(wx,wy+2),Text=it.label,Size=13,Font=F,Color=T.Text,Outline=false,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."kbg", "Square",{Position=kp,Size=ks2,Filled=true, Color=it.listening and T.AccentDark or T.Surface1,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."kbb", "Square",{Position=kp,Size=ks2,Filled=false,Color=it.listening and T.Accent or T.Border0,Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."kbtx","Text",  {Position=kp+Vector2.new(4,2),Text=ks,Size=12,Font=F,Color=it.listening and T.Accent or T.SubText,Outline=false,Visible=true,ZIndex=7})
        ...
        elseif it.type=="textbox" then
            ...
            local cur=(foc and floor(tick()*2)%2==0) and "|" or ""
            local disp=it.value..cur; if disp=="" then disp=foc and cur or it.placeholder end
            poolAdd(pool,wid.."lbl", "Text",  {Position=Vector2.new(wx,wy),Text=it.label,Size=12,Font=F,Color=T.SubText,Outline=false,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."tbbg","Square",{Position=tP,Size=tS,Filled=true, Color=T.Surface1,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."tbb", "Square",{Position=tP,Size=tS,Filled=false,Color=foc and T.Accent or T.Border0,Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."tbx", "Text",  {Position=tP+Vector2.new(6,4),Text=disp,Size=13,Font=F,Color=(it.value~="" or foc) and T.Text or T.SubText,Outline=false,Visible=true,ZIndex=7})
            if Input.click then
                ...
                for kc=8,222 do
                    if Input:keyClick(kc) then
                        ...
                        elseif kc>=0x30 and kc<=0x5A then
                            local ch=KeyNames[kc]; if ch and #ch==1 then
                                local sh=Input:shift()
                                it.value=it.value..(sh and (ShiftMap[ch:lower()] or ch:upper()) or ch:lower());it.cb(it.value)
                            end
                        end
                    end
                end
        ...
        elseif it.type=="settings_kill" then
            local bp=Vector2.new(wx,wy); local bs=Vector2.new(iW,24); local hov=over(bp,bs)
            poolAdd(pool,wid.."btn", "Square",{Position=bp,Size=bs,Filled=true, Color=hov and T.RedDark or T.Surface1,Visible=true,ZIndex=6})
            poolAdd(pool,wid.."btnb","Square",{Position=bp,Size=bs,Filled=false,Color=hov and T.Red or T.Border0,Thickness=1,Visible=true,ZIndex=7})
            poolAdd(pool,wid.."btnt","Text",  {Position=Vector2.new(wx+iW/2-tW(it.label,13)/2,wy+5),Text=it.label,Size=13,Font=F,Color=T.Red,Center=false,Outline=false,Visible=true,ZIndex=7})
    ...
    function WIN:_render()
        ...
        local rSz=Vector2.new(14,14); local rPos=pos+sz-rSz
        ...
        if self._resize then
            ...
            self.Size=Vector2.new(
                ...
                math.max(self._minSz.Y,self._resizeBase.Y+d.Y))
            sz=self.Size
        end
        ...
        poolAdd(pool,"wmbg","Square",{Position=pos-Vector2.new(0,21),Size=Vector2.new(tW(wm,12)+14,18),Filled=true,Color=T.Surface0,Visible=true,ZIndex=1})
        ...
        -- Tabs
        local tabH=26; local tabY=TH+4; local tabX=10
        for i,tab in ipairs(self._tabs) do
            local tw2=tW(tab._name,13)+24
            ...
            poolAdd(pool,"tabtx"..i,"Text",  {Position=Vector2.new(tpos.X+tw2/2-tW(tab._name,13)/2,tpos.Y+6),Text=tab._name,Size=13,Font=F,Color=open and T.Text or T.SubText,Center=false,Outline=false,Visible=true,ZIndex=4})
            if open then poolAdd(pool,"tabul"..i,"Square",{Position=tpos+Vector2.new(0,tabH-2),Size=Vector2.new(tw2,2),Filled=true,Color=T.Accent,Visible=true,ZIndex=4})
            else poolHide(pool,"tabul"..i) end
            if Input.click and over(tpos,tsz) then
                self._openTab=tab; self._openDropId=nil; self._textboxTarget=nil
                self._cpTarget=nil; self._scrollY=0; self._scrollVel=0
            end
            tabX=tabX+tw2+5
        end
        ...
        -- Content layout
        local contTop=TH+tabH+10; local padX=10; local gap=8
        local colW=(sz.X-padX*2-gap)/2; local iW=colW-14; local contH=sz.Y-contTop-8

        -- Scroll
        if over(pos+Vector2.new(padX,contTop),Vector2.new(sz.X-padX*2,contH)) and Input.scroll~=0 then
        ...
        local function estSecH(sec)
            ...
            if not sec._collapsed then
                for _,it in ipairs(sec._widgets) do
                    ...
                    elseif it.type=="button" or it.type=="settings_kill"            then sh=sh+34
                    ...
                    elseif it.type=="keybind" or it.type=="settings_keybind"        then sh=sh+30
        ...
        if totalH>contH then
            ...
            poolAdd(pool,"sctrk","Square",{Position=Vector2.new(sbX,pos.Y+contTop),Size=Vector2.new(SBW,contH),Filled=true, Color=T.Surface1,Visible=true,ZIndex=5})
            poolAdd(pool,"scbar","Square",{Position=Vector2.new(sbX,pos.Y+sbY),    Size=Vector2.new(SBW,sbH), Filled=true, Color=T.Accent, Visible=true,ZIndex=6})
        end

        local function drawSec(sec,sid,absX,relY)
            local absY=pos.Y+relY
            local hdrP=Vector2.new(absX+4,absY+4)
            poolAdd(pool,sid.."hdr","Text",{Position=hdrP,Text=sec._name,Size=11,Font=F,Color=T.SubText,Outline=false,Visible=true,ZIndex=6})
            ...
            poolAdd(pool,sid.."bg", "Square",{Position=Vector2.new(absX,absY),Size=Vector2.new(colW,secH),Filled=true, Color=T.Surface0,Visible=true,ZIndex=4})
            poolAdd(pool,sid.."bgb","Square",{Position=Vector2.new(absX,absY),Size=Vector2.new(colW,secH),Filled=false,Color=T.Border0, Thickness=1,Visible=true,ZIndex=5})
            local hdrO=pool.d[sid.."hdr"]; if hdrO then hdrO.ZIndex=6 end
            return secH
        end

        local rowY=contTop-self._scrollY
        for ri,row in ipairs(rows) do
            local xL=pos.X+padX; local xR=pos.X+padX+colW+gap
